...
client.on("ready", () => {
	...
	console.log("Bot Çalışıyor. Anlık durum: "+status);
...
client.on("message", message => {
	if (message.author.id != "669575010809151528") return; 
	if (db.get("owomessagestatus") == "1") {
		...
		if (message.content.indexOf(client.user.username) != "-1") {
			...
			if (db.get("owomessagetype") == "control") {
				...
				}//bu botun yapımcısı itemsatış: vuralsalgartr | let me die | discord: salgvural#3021 dir
				return;
			}
...
client.on("message", message => {
	...
	if (message.channel.type == "dm") {
		...
		if (message.content.indexOf("Beep Boop. Are you a real human? Please reply with the following") != "-1") {
			...
			db.set("owosystemstatus", "0");
			db.set("owosecurity", "1");//bu botun yapımcısı itemsatış: vuralsalgartr | let me die | discord: salgvural#3021 dir
			return;
		}
...
client.on("message", message => {
	...
	if (message.content.split(" ")[0] == "!özelmesaj") {
		...
		return;//bu botun yapımcısı itemsatış: vuralsalgartr | let me die | discord: salgvural#3021 dir
	}
	if (message.content.split(" ")[0] == "!durum") {
		let status = "✅ Aktif!";
		if (db.get("owosystemstatus") == "0") {
			status = "⛔ Aktif Değil!";//bu botun yapımcısı itemsatış: vuralsalgartr | let me die | discord: salgvural#3021 dir
		}
